...
class EditLayerDefs extends ui.modal.Panel {
	...
	function deleteLayer(ld:data.def.LayerDef, bypassConfirm = false) {
		if (!bypassConfirm && project.defs.isLayerSourceOfAnotherOne(ld)) {
			new ui.modal.dialog.Confirm(L.t._("Warning! This IntGrid layer is used by another one as SOURCE. Deleting it will also delete all rules in the corresponding auto-layer(s)!\n You may want to change these layers source to another one before..."),
				true, deleteLayer.bind(ld, true));
			return;
		}
	...
	function bakeLayer(ld:data.def.LayerDef) {
						...
						if (li.autoTilesCache.exists(r.uid)) {
							for (allTiles in li.autoTilesCache.get(r.uid).keyValueIterator())
								for (tileInfos in allTiles.value) {
									li.addGridTile(Std.int(tileInfos.x / ld.gridSize), Std.int(tileInfos.y / ld.gridSize), tileInfos.tid, tileInfos.flips);
								}
						}
					});
		...
		// Execute ops
		new Progress(L.t._("Baking layer instances"), ops, () -> {
		...
		if (cur.isAutoLayer()) {
			...
			jButton.click((_) -> {
				if (!cur.autoLayerRulesCanBeUsed())
					...
					new ui.modal.dialog.Confirm(jButton, L.t._("Transform this layer into a traditional Tile layer?"), true, () -> bakeLayer(cur));
			});
		}
		...
		switch cur.type {
			case IntGrid:
				...
				for (intGridVal in cur.getAllIntGridValues()) {
					...
					var i = new form.input.StringInput(e.find("input.name"), function() return intGridVal.identifier, function(v) {
						if (v != null && StringTools.trim(v).length == 0)
							v = null;
						intGridVal.identifier = data.Project.cleanupIdentifier(v);
					});
					...
					e.find("a.remove").click(function(ev) {
						...
						if (project.isIntGridValueUsed(cur, curIdx)) {
							new ui.modal.dialog.Confirm(e.find("a.remove"),
								L.t._("This value is used in some levels: removing it will also remove the value from all these levels. Are you sure?"), true,
								run);
							return;
						} else
							run();
			...
			case Tiles:
				...
				if (project.defs.tilesets.length == 0) {
					...
					if (td != null
						&& cur.gridSize != td.tileGridSize
						&& (td.tileGridSize < cur.gridSize || td.tileGridSize % cur.gridSize != 0)) {
