...
class Definitions {
	...
	public function isLayerNameUnique(id:String) {
		var id = Project.cleanupIdentifier(id);
		for (ld in layers)
			if (ld.identifier == id)
				return false;
		return true;
	...
	public function sortLayerAutoRules(ld:data.def.LayerDef, fromGroupIdx:Int, toGroupIdx:Int, fromRuleIdx:Int,
			toRuleIdx:Int):Null<data.def.AutoLayerRuleDef> {
		// Group list bounds
