...
class FieldDef {
	...
	@:keep public function toString() {
		return 'FieldDef.$identifier(' + (canBeNull ? 'Null<$type>' : '$type') + ', default=${getDefault()})'
	...
	public function getLongDescription() {
		...
		return getShortDescription()
			+ (canBeNull ? " (nullable)" : "")
			+ ", default = "
			+ ((type == F_String || type == F_Text) && getDefault() != null ? '"${getDefault()}"' : getDefault())
			+ (min == null
				&& max == null ? "" : (type == F_Int ? " [" + (min == null ? "-" + infinity : "" + dn.M.round(min)) + ";"
					+ (max == null ? "+" + infinity : "" + dn.M.round(max)) + "]" : "")
	...
	public function getEnumDef():Null<EnumDef> {
		return !isEnum() ? _project.defs.getEnumDef(switch type {
		...
		if (rawDef == null)
			...
			switch type {
				...
				case F_Bool:
					...
					if (rawDef == "true")
						defaultOverride = V_Bool(true);
					else if (rawDef == "false")
						defaultOverride = V_Bool(false);
					else
						defaultOverride = null;
	...
	function checkMinMax() {
		...
		switch defaultOverride {
			case V_Int(v):
				defaultOverride = V_Int(iClamp(v));
			case V_Float(v):
				defaultOverride = V_Float(fClamp(v));
			case _:
		}
	}
