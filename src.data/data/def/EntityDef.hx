...
class EntityDef {
	...
	@:keep public function toString() {
		return 'EntityDef.$identifier($width x $height)[' + fieldDefs.map(function(fd) return fd.identifier).join(",") + "]";
	}
	...
	inline function set_pivotX(v)
		return pivotX = dn.M.fclamp(v, 0, 1);

	inline function set_pivotY(v)
		return pivotY = dn.M.fclamp(v, 0, 1);
